...
만약 (1)~(3) 전체를 단일 `@Transactional` 로 묶으면 외부 PG 응답 대기 동안 DB 커넥션이 점유됩니다. 1000 TPS 피크와 PG 응답 1~3초가 만나면 HikariCP 풀이 폭발할 위험이 있습니다. InnoDB MVCC snapshot 누적, replication lag, row lock 보유 시간 연장 같은 부작용도 cascade합니다.
